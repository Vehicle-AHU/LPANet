...
class ComputeLoss:
    ...
    def __call__(self, p, targets, seg_loss=None):  # predictions, targets, model
        """
        Args:
        ...
        if seg_loss != None:
            rgb_attention_scores, ir_attention_scores, rgb_true, ir_true, weight_loss = seg_loss

            criterion = nn.BCEWithLogitsLoss()

            lsemrgb = sum(criterion(rgb_attention_scores[:, i:i+1, :, :], rgb_true[:, i:i+1, :, :]) for i in range(5))

            lsemir = sum(criterion(ir_attention_scores[:, i:i+1, :, :], ir_true[:, i:i+1, :, :]) for i in range(5))

            lsemrgb = (lsemrgb * 1.0).unsqueeze(0)
            lsemir = (lsemir * 1.0).unsqueeze(0)

            stacked_tensors = torch.stack(weight_loss)
            average_loss = torch.mean(stacked_tensors, dim=0)
            average_loss = (average_loss * 1.0).unsqueeze(0)
        ...
        if seg_loss != None:
            return (lbox + lobj + lcls + ltheta ) * bs + lsemrgb + lsemir + average_loss , torch.cat((lbox, lobj, lcls, ltheta, lsemrgb, lsemir, average_loss)).detach()
        else:
