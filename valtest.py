...
def run(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader_rgb_ir(val_path_rgb, val_path_ir,imgsz, batch_size // WORLD_SIZE * 2, gs, model.names, single_cls,
                                       hyp=None, cache=None, rect=False, rank=-1,
    ...
    for batch_i, (im, targets, _, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        out_list = model(img_rgb, img_ir,attr_vectors, batch_i)  # inference and training outputs

        out_old, _, _,_,_ = out_list
