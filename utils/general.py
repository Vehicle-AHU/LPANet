...
def check_git_status():
    ...
    msg = ', for updates see/ultralytics/yolov5'
...
def check_file(file, suffix=''):
    ...
    if Path(file).is_file() or file == '':  # exists
        ...
        file = Path(urllib.parse.unquote(file).split('?')[0]).name  # '%2F' to '/'
...
def url2file(url):
    # Convert URL to filename, i.e. -> file.txt
    url = str(Path(url)).replace(':/', '://')  # Pathlib turns :// -> :/
    file = Path(urllib.parse.unquote(url)).name.split('?')[0]  # '%2F' to '/', split
    return file
...
def one_cycle(y1=0.0, y2=1.0, steps=100):
    # lambda function for sinusoidal ramp from 
