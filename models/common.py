...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, img_rgb,img_ir, attr_vectors, batch_i, augment=False, visualize=False, val=False):
        # YOLOv5 MultiBackend inference
        im=img_rgb
        b, ch, h, w = im.shape  # batch, channel, height, width

        if self.pt or self.jit:  # PyTorch
            y = self.model(img_rgb,img_ir,attr_vectors, batch_i) if self.jit else self.model(img_rgb,img_ir, attr_vectors, batch_i, augment=augment, visualize=visualize)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        x_rgb,x_ir =  torch.split(x,N//2,dim=1)	

        x = x_rgb
        if self.sr_ratio > 1:

            x_ = x.permute(0, 2, 1).reshape(B, C, h,w)    
            x_ = self.sr_rgb(x_).reshape(B, C, -1).permute(0, 2, 1) 
            x_ = self.norm_rgb(x_)
        ...
        if self.sr_ratio > 1:
            ...
            x_ = x.permute(0, 2, 1).reshape(B, C, h,w)    
            x_ = self.sr_ir(x_).reshape(B, C, -1).permute(0, 2, 1) 
            x_ = self.norm_ir(x_)
        ...
        out_rgb_1,out_rgb_2 =  torch.split(out_rgb,N//2,dim=1)	
        out_ir_1,out_ir_2 =  torch.split(out_ir,N//2,dim=1)	
...
class myTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x =x_lpu.reshape(B, C, -1).permute(0, 2, 1 )
...
class CrossAttentionLayer(torch.nn.Module):
    def __init__(self, dim, num_heads, patch_num):
        super(CrossAttentionLayer, self).__init__()
        self.num_heads = num_heads
        self.dim = dim
        
        self.patch_num = patch_num
        self.grid_size = int(patch_num ** 0.5)  
        ...
        self.neighbor_indices = self.compute_neighbors()

    def compute_neighbors(self):
        padded_grid = torch.full((self.grid_size + 2, self.grid_size + 2), -1, dtype=torch.long) 
        idxs = torch.arange(self.patch_num).view(self.grid_size, self.grid_size)
        padded_grid[1:-1, 1:-1] = idxs

        neighbors = []
        for i in range(-1, 2):
            for j in range(-1, 2):
                shifted = padded_grid[1 + i:self.grid_size + 1 + i, 1 + j:self.grid_size + 1 + j]
                neighbors.append(shifted.unsqueeze(2)) 

        return torch.cat(neighbors, dim=2).view(-1, 9)
    
    def neighbors_rgb(self, rgb):

        batch_size, patch_num, dim = rgb.shape
        empty_patch = torch.zeros(batch_size, 1, dim, device=rgb.device).to(torch.float16)

        rgb_new = torch.cat((empty_patch, rgb), dim=1)

        neighbors = self.neighbor_indices.to(rgb.device)  # [patch_num, 9]
        neighbors = neighbors + 1
        neighbors = neighbors.unsqueeze(0).expand(batch_size, -1, -1)  # [batch_size, patch_num, 9]
        neighbors = neighbors.unsqueeze(-1).expand(-1, -1, -1, dim)  # [batch, patch_num, 9, dim]
        rgb_neighbors = torch.gather(rgb_new.unsqueeze(2).expand(-1, -1, neighbors.size(2), -1), 1, neighbors)  # [batch_size, patch_num, 9, dim]

        return rgb_neighbors


    def forward(self, rgb, ir):
        batch_size, patch_num, dim = rgb.shape
        
        query = self.query_proj(ir)  # [batch, patch_num, dim]

        rgb_neighbors = self.neighbors_rgb(rgb)

        key = self.key_proj(rgb_neighbors)  # [batch, patch_num, 9, dim]
        value = self.value_proj(rgb_neighbors)  # [batch, patch_num, 9, dim]

        attn_scores = torch.matmul(query.unsqueeze(2), key.transpose(-2, -1)) / (self.dim ** 0.5)  # [batch, patch_num, 1, 9]

        mask = (self.neighbor_indices == -1).unsqueeze(0).to(rgb.device)  # [1, patch_num, 9]
        mask = mask.expand(batch_size, -1, -1)  # [batch, patch_num, 9]
        mask = mask.unsqueeze(2)  

        attn_scores = attn_scores.masked_fill(mask, float('-inf'))  # [batch, patch_num, 1, 9]

        attn_weights = F.softmax(attn_scores, dim=-1)  # [batch, patch_num, 1, 9]

        updated_rgb = torch.matmul(attn_weights, value).squeeze(3).squeeze(2)  # [batch, num_heads, patch_num, head_dim]

        return self.output_proj(updated_rgb), attn_weights
...
class GPTcross(nn.Module):
    ...
    def __init__(self, d_model,sr_ratio, vert_anchors, horz_anchors,n_layer, 
        ...
        self.CrossAttentionLayer = CrossAttentionLayer(d_model, 4, vert_anchors * horz_anchors)
    ...
    def forward(self, x):
        ...
        rgb_align_feature, weight_ir_rgb = self.CrossAttentionLayer(rgb_token_embeddings, ir_token_embeddings)
        ir_align_feature,_ = self.CrossAttentionLayer(ir_token_embeddings, rgb_token_embeddings)
        _, weight_rgb_ir = self.CrossAttentionLayer(rgb_token_embeddings, rgb_align_feature)
        rgb_token_embeddings = (rgb_token_embeddings + rgb_align_feature) #/ 2
        #ir_token_embeddings = (ir_token_embeddings + ir_align_feature) / 2

        weight_loss = self.kl_divergence_loss(weight_rgb_ir, weight_ir_rgb)
        ...
        return [rgb_fea_out, ir_fea_out], weight_loss

    def kl_divergence_loss(self, attn_weights_1, attn_weights_2):

        attn_weights_1 = F.softmax(attn_weights_1, dim=-1)  # [batch, patch, 1, 9]
        attn_weights_2 = F.softmax(attn_weights_2, dim=-1)  # [batch, patch, 1, 9]
        
        attn_weights_1 = attn_weights_1.view(attn_weights_1.shape[0], -1)  # [batch*patch, 9]
        attn_weights_2 = attn_weights_2.view(attn_weights_2.shape[0], -1)  # [batch*patch, 9]
        
        loss = F.kl_div(torch.log(attn_weights_1), attn_weights_2, reduction='batchmean')
