...
class LoadMultiModalImagesAndLabels(Dataset):
    ...
    def __init__(self, path_rgb,path_ir, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=True,
        ...
        #label_ir
        self.labels_2 = self.labels_ir
        self.shapes_2 = self.shapes_ir
        self.indices_2 = self.indices_ir
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            img_rgb,img_ir, (h0, w0), (h, w), img_label, img_label_2 = load_image_rgb_ir(self, index) 
            ...
            img_ir, ratio_2, pad_2 = letterbox(img_ir, shape, auto=False, scaleup=self.augment)
            ...
            labels_2 = self.labels_ir[index].copy() # labels (array): (num_gt_perimg, [cls_id, poly])
            if labels_2.size:  
                # labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
                labels_2[:, [1, 3, 5, 7]] = img_label_2[:, [1, 3, 5, 7]] * ratio_2[0] + pad_2[0]
                labels_2[:, [2, 4, 6, 8]] = img_label_2[:, [2, 4, 6, 8]] * ratio_2[1] + pad_2[1]

            labels_rgb = labels
            labels_ir = labels_2

        nL = len(labels_rgb)  # number of labels
        nL_2 = len(labels_ir)  # number of labels
        # if nl:
        ...
        if self.augment:
            ...
            img_h_ir, img_w_ir = img_ir.shape[0], img_ir.shape[1]
            # Flip up-down
            if random.random() < hyp['flipud']:
                ...
                #if nL_2:
                    labels_ir[:, 2::2] = img_h_ir - labels_ir[:, 2::2] - 1
            ...
            if random.random() < hyp['fliplr']:
                ...
                #if nL_2:
                    labels_ir[:, 1::2] = img_w_ir - labels_ir[:, 1::2] - 1
        ...
        if nL_2:
            rboxes_2, csl_labels_2  = poly2rbox(polys=labels_ir[:, 1:], 
                                            num_cls_thata=hyp['cls_theta'] if hyp else 180, 
                                            radius=hyp['csl_radius'] if hyp else 6.0, 
                                            use_pi=True, use_gaussian=True)
            labels_obb_2 = np.concatenate((labels_ir[:, :1], rboxes_2, csl_labels_2), axis=1)
            labels_mask_2 = (rboxes_2[:, 0] >= 0) & (rboxes_2[:, 0] < img_ir.shape[1]) \
                        & (rboxes_2[:, 1] >= 0) & (rboxes_2[:, 0] < img_ir.shape[0]) \
                        & (rboxes_2[:, 2] > 5) | (rboxes_2[:, 3] > 5)
            labels_obb_2 = labels_obb_2[labels_mask_2]
            nL_2 = len(labels_obb_2)  # update after filter
        ...
        labels_out_2 = torch.zeros((nL_2, c_num))
        if nL:
            # labels_out[:, 1:] = torch.from_numpy(labels)
            labels_out[:, 1:] = torch.from_numpy(labels_obb)
        if nL_2:
            labels_out_2[:, 1:] = torch.from_numpy(labels_obb_2)
        ...
        return torch.from_numpy(img_all), labels_out, labels_out_2, self.img_files_rgb[index], shapes

    @staticmethod
    def collate_fn(batch):
        img, label, label_2, path, shapes = zip(*batch)  # transposed; (tupe(b*tensor))
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        for j, l_r in enumerate(label_2):
            l_r[:, 0] = j  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), torch.cat(label_2, 0), path, shapes

    @staticmethod
    def collate_fn4(batch):
        img, label, label_2, path, shapes = zip(*batch)  # transposed
        n = len(shapes) // 4
        img4, label4, label4_2, path4, shapes4 = [], [], [], path[:n], shapes[:n]
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                ...
                l = torch.cat((label_2[i], label_2[i + 1] + ho, label_2[i + 2] + wo, label_2[i + 3] + ho + wo), 0) * s
            img4.append(im)
            label4.append(l)
            label4_2.append(l_r)
        ...
        for j, l_r in enumerate(label4_2):
            l_r[:, 0] = j  # add target image index for build_targets()

        return torch.stack(img4, 0), torch.cat(label4, 0), torch.cat(label4_2, 0), path4, shapes4
...
def load_image_rgb_ir(self, i):
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        if r != 1:  # if sizes are not equal
            ...
            label[:, 1:] *= r
            label_2[:, 1:] *= r
        return img_rgb, img_ir,(h0, w0), img_rgb.shape[:2], label, label_2  # im, hw_original, hw_resized, resized_label
    else:
        return self.imgs_rgb[i], self.imgs_ir[i], self.img_hw0_rgb[i], self.img_hw_rgb[i] , self.labels[i], self.labels_2[i]
...
def load_mosaic_RGB_IR(self, index1, index2):#1
    ...
    for i, index in enumerate(indices):
        # Load image
        img_rgb, img_ir, _, (h, w),img_label, img_label_2 = load_image_rgb_ir(self, index)
        ...
        labels_ir, segments_ir = img_label_2.copy(), self.segments_ir[index].copy() # labels (array): (num_gt_perimg, [cls_id, poly])
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False, profile=False, hub=False):
    ...
    for split in 'train', 'val', 'test':
        ...
        for label in tqdm(dataset.labels, total=dataset.n, desc='Statistics'):
            x.append(np.bincount(label[:, 0].astype(np.int_), minlength=data['nc']))
...
def random_perspective_rgb_ir(img_rgb, img_ir, targets_rgb=(),targets_ir=(), segments_rgb=(), segments_ir=(),
    ...
    im = img_rgb
    targets = targets_rgb
    targets_2 = targets_ir
    segments = segments_rgb
    segments_2 = segments_ir
    ...
    m = len(targets_2)
    if m:
        use_segments_2 = any(x.any() for x in segments_2)
        new_2 = np.zeros((m, 4))
        if use_segments_2:  # warp segments
            segments_2 = resample_segments(segments_2)  # upsample
            for i, segment_2 in enumerate(segments_2):
                xy_2 = np.ones((len(segment_2), 3))
                xy_2[:, :2] = segment_2
                xy_2 = xy_2 @ M.T  # transform
                xy_2 = xy_2[:, :2] / xy_2[:, 2:3] if perspective else xy_2[:, :2]  # perspective rescale or affine

                # clip
                new_2[i] = segment2box(xy_2, width, height)

        else:  # warp boxes
            xy_2 = np.ones((m * 4, 3))
            # xy[:, :2] = targets[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy_2[:, :2] = targets_2[:, 1:].reshape(m * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy_2 = xy_2 @ M.T  # transform
            xy_2 = (xy_2[:, :2] / xy_2[:, 2:3] if perspective else xy_2[:, :2]).reshape(m, 8)  # perspective rescale or affine

        targets_mask_2 = poly_filter(polys=xy_2, h=height, w=width)
        targets_2[:, 1:] = xy_2
        targets_2 = targets_2[targets_mask_2]

    return img_rgb, img_ir, targets, targets_2
