...
def run(data,
    ...
    attr_vectors = dataset_info.attr_vectors.astype(np.float32)
    ...
    for batch_i, (im, targets, _, paths, shapes) in enumerate(pbar):
        t1 = time_sync()
        if pt or jit or engine:
            im = im.to(device, non_blocking=True)
            targets = targets.to(device)
        im = im.half() if half else im.float()  # uint8 to fp16/32
        attr_vectors = attr_vectors.half() if half else attr_vectors.float()  # uint8 to fp16/32
        ...
        # Inference
        out_old = model(img_rgb,img_ir,attr_vectors) if training else model(img_rgb, img_ir, attr_vectors, augment=augment, val=True)  # inference, loss outputs

        out_new,_,_,_,_ = out_old
        out, train_out= out_new
